...
def file_check(repeat_library, in_gff, genome, out_gff, temp_dir):
    ...
    if not exists(temp_dir):
        os.mkdir(temp_dir)
    if not exists(os.path.join(temp_dir, "qseqs")):
        os.mkdir(os.path.join(temp_dir, "qseqs"))
    if not exists(os.path.join(temp_dir, "split_library")):
...
if __name__ == "__main__":
    if multiprocessing.get_start_method(allow_none=True) is None:
        try:
            ...
            # Fall back to the existing/default start method if fork is unavailable
